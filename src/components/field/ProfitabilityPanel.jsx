...
const ProfitabilityPanel = ({ fieldId }) => {
    ...
    useEffect(() => {
        const fetchProfit = async () => {
            try {
                ...
                console.warn('Profit estimation unavailable, showing demo data:', err.message);
                setProfitData(MOCK_PROFIT);
            } finally {
                setLoading(false);
            }
        };
    ...
    const scoreColor =
        profitData.profit_score > 70 ? 'bg-emerald-500' :
            profitData.profit_score > 40 ? 'bg-yellow-500' : 'bg-red-500';

    const scoreBadge =
        profitData.profit_score > 70 ? 'bg-green-100 text-green-700' :
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
                ...
                <span className={`text-xs font-black px-2 py-0.5 rounded ${scoreBadge}`}>
            ...
            <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div
                    className={`h-full transition-all duration-1000 ${scoreColor}`}
