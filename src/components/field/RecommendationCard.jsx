...
const RecommendationCard = ({ recommendation, fieldId }) => {
  ...
  // Auto-fetch reasoning on mount for supported cards
  useEffect(() => {
  ...
  // Re-fetch reasoning when language changes
  useEffect(() => {
  ...
  return (
    <div className={`card border-2 ${isCritical ? 'border-red-300 bg-red-50' : ''}`}>
      ...
      {/* Section 2: AI Reasoning (Automated & Collapsible) - Only for supported cards */}
      {supportsReasoning && (
