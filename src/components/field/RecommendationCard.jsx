      ...
      <div className="pt-3 border-t-2 border-dashed border-gray-100">
        ...
        {!isCollapsed && (
          <div className="animate-in fade-in slide-in-from-top-1 duration-300">
            ...
            {reasoning && !loading && (
              <div className="mt-2 bg-gradient-to-br from-primary-50/30 to-white border border-primary-100/50 rounded-xl p-4 shadow-sm backdrop-blur-sm overflow-hidden border-l-4 border-l-primary-400">
                <ReactMarkdown
                  components={{
                    h3: ({ node, ...props }) => {
                      ...
                      if (title.includes('Action')) icon = <svg className="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
                      ...
                      if (title.includes('Timing')) icon = <svg className="w-4 h-4 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;

                      return (
                        <div className="flex items-center gap-1.5 mb-2 mt-4 first:mt-0 pb-1 border-b border-gray-100/50">
                          {icon}
                          <h3 className="text-[10px] font-black text-gray-800 uppercase tracking-widest" {...props} />
                        </div>
                      );
                    },
                    p: ({ node, ...props }) => {
                      return <p className="text-gray-700 leading-relaxed mb-3 font-medium text-[13px]" {...props} />;
                    },
                    ul: ({ node, ...props }) => <ul className="space-y-1.5 mb-4" {...props} />,
                    li: ({ node, ...props }) => (
                      <li className="flex items-start gap-2 bg-gray-50/50 p-2 rounded-lg border border-gray-100/50 transition-all hover:bg-white shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
                        <span className="w-1 h-1 rounded-full bg-primary-400 mt-1.5 shrink-0" />
                        <span className="text-gray-600 text-[12px] font-medium leading-relaxed" {...props} />
                      </li>
                    ),
                    strong: ({ node, ...props }) => <strong className="font-bold text-primary-900 bg-primary-50 px-1 py-0.5 rounded-md border border-primary-100 mx-0.5" {...props} />
                  }}
                >
                  {reasoning}
                </ReactMarkdown>
